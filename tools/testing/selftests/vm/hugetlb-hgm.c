		   ...
		   bool verify)
{
	...
	for (uint64_t size = len/2; size >= pagesize;
			offset += size, size /= 2) {
		iter++;
...
int test_collapse(char *primary_map, size_t len, bool hwpoison)
{
	...
	size_t offset = 0;
	size_t iter = 0;
	for (uint64_t size = len/2; size > 4096; offset += size, size /= 2) {
		iter++;
		if (check_equal(primary_map + offset, size, iter))
			return -1;
	}
...
int test_poison_sigbus(char *addr)
{
	...
	pthread_t pthread;
	if ((ret = pthread_create(&pthread, NULL, &access_mem, addr))) {
...
int test_hwpoison(char *primary_map, size_t len)
{
	...
	for (int i = 0; i < len;
			i += MAX(pagesize, len/num_poison_checks))
...
int test_hgm(int fd, size_t hugepagesize, size_t len, bool hwpoison)
{
	...
	char *primary_map = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	if (primary_map == MAP_FAILED) {
		perror("mmap for primary mapping failed");
		ret = -1;
		goto close_uffd;
	}
	char *secondary_map = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
{
	if (argc != 2) {
		const char *name = argc > 0 ? argv[0] : "hugetlb-hgm";
		printf("usage: %s <hugetlbfs-file>\n", name);
		return -1;
	}
	...
	if (file_stat.f_type != HUGETLBFS_MAGIC) {
		printf("not hugetlbfs mount\n");
		goto close;
	}

	const size_t hugepagesize = file_stat.f_bsize;
	const size_t len = 2 * hugepagesize;

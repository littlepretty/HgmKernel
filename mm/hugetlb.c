...
retry_avoidcopy:
	...
	if (IS_ERR(new_page)) {
		...
		if (outside_reserve) {
			...
			if (likely(ptep)) {
				hugetlb_pte_populate(mm, &new_hpte, ptep,
						huge_page_shift(h),
						hpage_size_to_level(
							huge_page_size(h)));
	...
	ptep = hugetlb_walk(vma, haddr, huge_page_size(h));
	if (likely(ptep))
		hugetlb_pte_populate(mm, &new_hpte, ptep, huge_page_shift(h),
		      ...
		      unsigned long addr, pud_t *pud)
{
	...
	hugetlb_pte_populate(mm, &hpte, (pte_t *)pud, PUD_SHIFT,
			HUGETLB_LEVEL_PUD);
	spin_lock(hpte.ptl);
	if (pud_none(*pud)) {
		pud_populate(mm, pud,
				...
				unsigned long addr)
{
	...
	hugetlb_hgm_walk(vma->vm_mm, vma, hpte, addr, PAGE_SIZE, false);
}
 ...
 * must also not be mapped in that case.
 *
		      ...
		      unsigned long addr)
{
	...
	hugetlb_hgm_walk_uninit(hpte, ptep, vma, addr, PAGE_SIZE, false);
	return 0;
}
				   ...
				   unsigned long target_sz)
{
	...
	if (!target_sz) {
		hugetlb_hgm_walk_uninit(hpte, ptep, vma, addr, PAGE_SIZE,
				false);
		return 0;
	}

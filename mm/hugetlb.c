...
static void hugetlb_dup_file_rmap(struct *page page, unsigned long sz)
{
	struct page *hpage = compound_head(page);
	unsigned long nr_pages = sz / PAGE_SIZE;
			...
			pte_t old_pte, unsigned int flags)
{
	...
	if (anon_rmap)
		...
		hugetlb_dup_file_rmap(page, hugetlb_pte_size(hpte));
			    ...
			    bool wp_copy)
{
	...
	if (page_in_pagecache)
		hugetlb_dup_file_rmap(page, hugetlb_pte_size(dst_hpte));
	else
			    ...
			    unsigned long start, unsigned long end)
{
	...
	while (curr < end) {
		...
		hugetlb_dup_file_rmap(subpage, hugetlb_pte_size(&hpte));
...
next_hpte:
		...
		if (curr < end) {
			/* Don't hold the locks for too long. */
			i_mmap_unlock_write(vma->vm_file->f_mapping);
			hugetlb_vma_unlock_write(vma);
			cond_resched();
			hugetlb_vma_lock_write(vma);

			 ...
			 struct hstate *h, struct vm_area_struct *vma)
{
	...
	if (shift == huge_page_shift(h)) {
		...
		page_remove_rmap(subpage, vma, true);
	} else {
		unsigned long nr_subpages = 1UL << (shift - PAGE_SHIFT);
		struct page *final_page = &subpages[nr_subpages];

		VM_BUG_ON_PAGE(VmemmapOptimized(hpage), hpage);
		/*
				   ...
				   struct page *ref_page, zap_flags_t zap_flags)
{
	...
	while (address < end) {
		...
		hugetlb_remove_rmap(hpage, hpte.shift, h, vma);
			...
			pte_t old_pte, unsigned int flags)
{
	...
	if (anon_rmap) {
		VM_BUG_ON_PAGE(page != subpage, subpage);
			    ...
			    unsigned long start, unsigned long end)
{
	...
	while (curr < end) {
		...
		hugetlb_dup_file_rmap(subpage, hpte.shift, h, vma);

...
hugetlb_level_t hpage_size_to_level(unsigned long sz) {
	 ...
	 *
	 * @sz must always be at most the size of the largest supported
	 * hugepage size, and all page table levels at or below that size
	 * should never be folded, so WARN in those cases.
	 */
	if (sz < PMD_SIZE)
...
retry:
	...
	else if (!huge_pte_none(hugetlb_pte_get(hpte)))
		/*
...
static void hugetlb_vm_op_open(struct vm_area_struct *vma)
{
	...
	/*
	 * hugetlb_shared_vma_data structure for sharable mappings is vma
	 * specific. Clear old pointer (if copied via vm_area_dup) and create
	 * new.
	 */
	if (vma->vm_flags & VM_MAYSHARE) {
		     ...
		     unsigned long addr, struct page *new_page)
{
	...
	set_hugetlb_pte_at(vma->vm_mm, addr, hpte,
...
again:
		if (huge_pte_none(entry)) {
			...
			set_hugetlb_pte_at(dst, addr, &dst_hpte, entry);
			...
			if (!is_readable_migration_entry(swp_entry) && cow) {
				...
				set_hugetlb_pte_at(src, addr, &src_hpte, entry);
			}
			if (!userfaultfd_wp(dst_vma) && uffd_wp)
				entry = huge_pte_clear_uffd_wp(entry);
			set_hugetlb_pte_at(dst, addr, &dst_hpte, entry);
			     ...
			     struct hugetlb_pte *dst_hpte)
{
	...
	set_hugetlb_pte_at(mm, new_addr, dst_hpte, pte);
	...
	while (address < end) {
		...
		if (unlikely(!pte_present(pte))) {
			...
			if (pte_swp_uffd_wp_any(pte) &&
				...
				set_hugetlb_pte_at(mm, address, &hpte,
...
retry_avoidcopy:
	...
	if (likely(ptep && pte_same(hugetlb_pte_get(&new_hpte), pte))) {
		...
		set_hugetlb_pte_at(mm, haddr, &new_hpte,
			       ...
			       struct hugetlb_pte *hpte, pte_t old_pte)
{
	...
	ptl = hugetlb_pte_lock(mm, hpte);
	same = pte_same(hugetlb_pte_get(hpte), old_pte);
	spin_unlock(ptl);
			...
			pte_t old_pte, unsigned int flags)
{
	...
	set_hugetlb_pte_at(mm, haddr_hgm, hpte, new_pte);
			    ...
			    bool wp_copy)
{
	...
	set_hugetlb_pte_at(dst_mm, dst_addr, dst_hpte, _dst_pte);
	...
	while (address < end) {
		...
		if (unlikely(is_hugetlb_entry_migration(pte))) {
			...
			if (!is_readable_migration_entry(entry)) {
				...
				set_hugetlb_pte_at(mm, address, &hpte, newpte);
				base_pages += hugetlb_pte_size(&hpte) / PAGE_SIZE;
			}
			spin_unlock(ptl);
			goto next_hpte;
		...
		if (!huge_pte_none(pte)) {
			...
			if (unlikely(!hugetlb_pte_present_leaf(&hpte, pte))) {
				spin_unlock(ptl);
				WARN_ONCE(1, "Found unexpected non-leaf PTE: "
					     "ptep:%p, address:0x%lx\n",
					     hpte.ptep, address);
				continue;
			}
		      ...
		      unsigned long addr, unsigned long sz)
{
	...
	switch(hpte->level) {
		...
		case HUGETLB_LEVEL_PMD:
			...
			break;
		default:
			WARN_ONCE(1, "HugeTLB: HGM walk got invalid level: %d "
...
int enable_hugetlb_hgm(struct vm_area_struct *vma)
{
	 ...
	 *
	 * Callers also rely on hugetlb_hgm_enabled not changing while holding
			    ...
			    unsigned long start, unsigned long end)
{
	...
	while (curr < end) {
		...
		/*
		 * Because we're holding the VMA lock for writing, it's
		 * impossible for the PTEs to change from under us, so it
		 * will be impossible for for us to blow away hwpoison or
		 * migration PTEs.
		 */
		...
		set_hugetlb_pte_at(mm, curr, &hpte, entry);
next_hpte:
		curr += hugetlb_pte_size(&hpte);

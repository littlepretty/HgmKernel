...
static void hugetlb_vma_data_alloc(struct vm_area_struct *vma);
...
again:
		if (huge_pte_none(entry)) {
			...
			if (userfaultfd_wp(dst_vma))
				set_huge_pte_at(dst, addr, dst_pte, entry);
		} else if (!hugetlb_pte_present_leaf(&src_hpte)) {
			/* Retry the walk. */
			spin_unlock(src_ptl);
			spin_unlock(dst_ptl);
			continue;
...
bool hugetlb_hgm_eligible(struct vm_area_struct *vma)
{
	...
	return flags & VM_MAYSHARE && flags & VM_HUGETLB;
}
bool hugetlb_hgm_enabled(struct vm_area_struct *vma)
{
	return hugetlb_hgm_eligible(vma);
}

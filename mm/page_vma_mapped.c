 ...
 * adjusted if needed (for PTE-mapped THPs and subpage-mapped HugeTLB pages).
 *
...
bool page_vma_mapped_walk(struct page_vma_mapped_walk *pvmw)
{
	...
	if (unlikely(is_vm_hugetlb_page(vma))) {
		...
		do {
			...
			/*
			 * Do a high granularity page table walk. The i_mmap_rw_sem is
			 * dropped in page_vma_mapped_walk_done(). We need to hold it
			 * so that high-granularity PTEs stay valid.
			 */
			if (!pvmw->pte)

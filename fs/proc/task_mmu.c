...
struct mem_size_stats {
	...
	/* Keyed by the hstate index corresponding to the mapping size. */
	unsigned long hugetlb_hgm_mapped[HUGE_MAX_HSTATE];
#endif
	u64 pss;
	u64 pss_anon;
	u64 pss_file;
		...
		struct hugetlb_pte *hpte)
{
	...
	mss->hugetlb_hgm_mapped[hstate_index(size_to_hstate(size))] += size;
#endif
}
	...
	bool rollup_mode)
{
	...
	for_each_hstate(h) {
		seq_puts(m, " kB\n");
		SEQ_PUT_DEC("Hugetlb-", huge_page_size(h));
		SEQ_PUT_DEC("kB_Mapped: ",
				mss->hugetlb_hgm_mapped[hstate_index(h)]);
	}
#endif
	SEQ_PUT_DEC(" kB\nSwap:             ", mss->swap);
	SEQ_PUT_DEC(" kB\nSwapPss:          ",
